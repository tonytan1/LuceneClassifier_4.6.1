...
/**Supervised machine learning algorithm (Naive Bayes Model) to auto-classify
 * bug titles.
 * 
 * @author martin.wang
 *
...
public class ClassifierUtils {
	...
	public static final String[] CATEGORIES = { "error",
			"user", "module", "report", "editor",
			"exam", "learning", "catalog", "session" };
	...
	public static void runNaiveBayesClassifier(){
		...
		try {
			...
			/*classifier.train(ar, "Category", "Summary", new StandardAnalyzer(
					Version.LUCENE_46));*/
			classifier.train(ar, "Summary", "Category", new StandardAnalyzer(
					Version.LUCENE_46)); //training
			...
			for (int i = 0; i < maxdoc; i++) {
				...
				ClassificationResult<BytesRef> result = classifier.assignClass(doc
						.get("Summary")); //label the result
				...
				/*	if(cli == -1){
						System.out.println("Unknown category:" + classified);	
					}
					
					if(cai == -1){
						System.out.println("Unknown summary:" + correctAnswer);
					}*/
				}
			}
			...
			// print results
			String result = SearchEngineUtils.printConfusionMatrix(CATEGORIES, counts);
			boolean overwrite = true;
			String filePath = "./resource/NaiveBayesClassification.txt";
